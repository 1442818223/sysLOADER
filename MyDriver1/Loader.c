...
EXTERN_C PVOID RtlFindExportedRoutineByName(  //根据指定模块和函数名称，返回该函数在内存中的基址。
	PVOID BaseAddress,
	PCSTR RoutineName
);
typedef NTSTATUS(*DriverEntryProc)(PDRIVER_OBJECT pDriver, PUNICODE_STRING pReg);
...
PUCHAR FileToImage(char* fileBuffer)
{
	...
	PIMAGE_DOS_HEADER pDos = (PIMAGE_DOS_HEADER)fileBuffer;  //这个结构在32位和64位系统中是相同的
	PIMAGE_NT_HEADERS pNts = (PIMAGE_NT_HEADERS)(fileBuffer + pDos->e_lfanew);  //PIMAGE_NT_HEADERS自动分
...
VOID UpdateCookie(char* imageBuffer)
{
	...
	*(PULONG_PTR)(config->SecurityCookie) += 10;         // PULONG是4字节  PULONG_PTR是8字节
...
BOOLEAN LoadDriver(PUCHAR fileBuffer, PDRIVER_OBJECT pDriver, PUNICODE_STRING pReg)
{
	...
	do
	{
		...
		NTSTATUS status = EntryPointFunc(pDriver, pReg); //入口点直接执行  //没有驱动对象 被加载的也不能用
